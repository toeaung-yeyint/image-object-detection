<template>
  <div class="max-w-screen-lg my-28 mx-auto px-8">
    ...
    <div v-show="result" class="flex flex-col lg:flex-row items-center">
      <div
        class="relative w-full lg:w-2/3 mb-8 lg:mb-0"
        ref="resultImageContainer"
      >
        <img class="w-full" :src="src" alt="" ref="resultImage" />
        <span
          class="text-[#00ff00] cursor-default text-xs sm:text-base lg:font-medium lg:inline-block border-2 border-[#00ff00] absolute lg:bg-black lg:bg-opacity-20 lg:hover:bg-opacity-30"
...
onMounted(() => {
  window.addEventListener("resize", () => {
    if (result.value) {
      scalingFactor.value = resultImageContainer.value.offsetWidth / 640;
    }
  });
});
